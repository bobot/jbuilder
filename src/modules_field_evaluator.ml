...
let find_errors ~modules ~intf_only ~virtual_modules ~private_modules =
  let add_property prop m =
    Module.Name.Map.map m
      ~f:(fun (loc,module_) -> module_, Properties.Map.singleton prop loc)
  in
  let modules = add_property Modules modules in
  let intf_only = add_property Intf_only intf_only in
  let virtual_modules = add_property Virtual virtual_modules in
  let private_modules = add_property Private private_modules in
  let union m1 m2 =
    Module.Name.Map.union m1 m2
      ~f:(fun _ (module_,l1) (_,l2) ->
        Some (module_,Properties.Map.union l1 l2 ~f:(fun _ _ -> assert false) ))
  in
  let all = union modules (union intf_only (union virtual_modules private_modules)) in
  ...
  Module.Name.Map.iteri all
    ~f:(fun module_name (module_,props) ->
...
let check_invalid_module_listing ~(buildable : Buildable.t) ~intf_only
  ...
  let errors = find_errors ~modules ~intf_only ~virtual_modules ~private_modules in
  let uncapitalized =
    List.map ~f:(fun (_, m) -> m |> Module.Name.uncapitalize) in
